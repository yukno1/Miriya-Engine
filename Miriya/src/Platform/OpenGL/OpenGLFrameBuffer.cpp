...
void OpenGLFramebuffer::Invalidate()
{
    ...
    if (m_ColorAttachments.size() > 1) {
        MIR_CORE_ASSERT(m_ColorAttachments.size() <= 4, "Color attachment index out of range");
        GLenum buffers[4] = {
    ...
    MIR_CORE_ASSERT(glCheckFramebufferStatus(GL_FRAMEBUFFER) == GL_FRAMEBUFFER_COMPLETE,
...
void OpenGLFramebuffer::Resize(uint32_t width, uint32_t height)
{
    if (width == 0 || height == 0 || width > s_MaxFramebufferSize ||
        ...
        MIR_CORE_WARN("Attempted to rezize framebuffer to {0}, {1}", width, height);
        return;
    }
